...
| 목록 (메인 / 삭제된 항목 탭) | `/` |
| 설정 | `/settings` |
| 개인정보처리방침 | `/privacy` (라우트·템플릿 구현 완료, 실제 공개는 운영 정보 확정·법률 검토 후) |
| 이용약관 | `/terms` (라우트·템플릿 구현 완료, 실제 공개는 운영 정보 확정·법률 검토 후) |
...
- `/privacy`, `/terms` 라우트(`routes/site.py`)와 템플릿
  (`templates/privacy.html`, `templates/terms.html`)은 구현되어 있으며
  로그인 없이 열 수 있다. 로그인 화면 하단과 설정 화면에서 링크를 제공한다.
- 운영 주체명·문의 이메일·서비스 주소·시행일은 코드에 하드코딩하지 않고
  `SERVICE_OPERATOR` / `PRIVACY_CONTACT_EMAIL` / `SERVICE_URL` /
  `POLICY_EFFECTIVE_DATE` 환경 변수로 주입한다(`services/site_info.py`).
...
| Cloud Run 배포 구성 | ✅ |
| 헤더 버전·사용자명 표시 | ✅ |
| `/privacy`, `/terms` 라우트·템플릿 및 로그인 화면 링크 | 🟡 (코드 완료, 콘텐츠 확정·법률 검토 후 실제 공개) |
| PWA (manifest, favicon, 홈 화면 아이콘) | ✅ |
| 초기 로딩 표시 ("Now Loading") | ✅ |
| 앱 스타일 확인 모달 (삭제·로그아웃·가져오기) | ✅ |
| 가져오기 전 영향 범위(추가/중복 건수) 미리 확인 | ✅ |
