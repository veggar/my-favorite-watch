...
## 7. 진행 상황 업데이트 (2026-08-20)

이 문서 작성(v1.2.0) 이후 P0-1, P0-2 착수, P0-3, P0-5, P0-6, HMAC 사용자 키
분리, 완전한 서버 측 세션(v1.6.0)까지는 `master`에 정상 반영되었다. 그런데
**P0-4·P1-2·P1-7·P2-1~4를 구현한 브랜치(`claude/release-readiness-p0p1p2`,
커밋 `b4cd271`)가 PR도 없이 로컬에만 남아 `master`에 병합되지 않은 채
방치되어 있었다** — 정확히 위 "브랜치 · PR 규칙"이 경고하는 사고 패턴이다.

2026-08-20에 이를 발견해 다음과 같이 정리했다.

- **P1-2(OAuth 오류 처리), P1-7(TMDb 비활성 표시)**: 그 사이 `master`가 더
  정교한 버전(`AUTH_HOST`, `AUTH_COOKIE_BLOCKED` 등 PRD §4.2 코드 체계)을
  이미 갖추고 있어 포팅하지 않았다.
- **P0-4(개인정보처리방침·이용약관), P2-1(PWA), P2-2(초기 로딩 표시),
  P2-3(확인 모달), P2-4(가져오기 안전장치)**: 방치된 브랜치를 참고자료로
  삼아 현재 `master`의 세션 구조(HMAC 사용자 키, 서버 측 세션)에 맞게
  새 브랜치(`claude/release-readiness-p0-p2`)로 다시 옮겼다. `/privacy`,
  `/terms` 콘텐츠는 이후 작성된 `docs/legal/*.md` 초안을 기준으로 새로
  작성했다(운영 주체 정보는 환경 변수 4종으로 주입).
- **P1-1(테스트 부재)**: import 안전장치, 공개 라우트, PWA/UI 마크업,
  CSV 헤더 판정 회귀(신규 발견 버그) 테스트를 추가했다.

방치됐던 브랜치 자체는 삭제하지 않고 참고용으로 남겨둔다. 실제 반영 여부는
`git log --oneline master..<branch>`로 다시 확인하지 않는 한 PR 목록이나
커밋 메시지만으로 판단하지 않는다.
