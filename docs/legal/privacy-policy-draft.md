...
> 상태: 콘텐츠 검토용 초안 — `/privacy`에 아직 공개되지 않음  
> 기준일: 2026-08-20  
> 시행일: [운영자 입력 필요]  
...
| 파일 가져오기 | 이용자가 업로드한 CSV·Excel 내용과 파싱 결과 | 미리보기와 Google Sheet 일괄 등록 | 미리보기 후 등록 완료 시까지. 현재 파싱 결과가 암호화되지 않은 서명 쿠키에 임시 저장되므로 서버 측 단기 저장으로 이전하고 삭제 시점을 검증해야 함 |
...
만 14세 미만 아동을 서비스 대상으로 할지 여부와 법정대리인 동의 절차는
[운영자 결정 필요]입니다. 절차가 확정되기 전에는 만 14세 미만의 이용을
허용하지 않는 것을 운영 기준으로 합니다.
